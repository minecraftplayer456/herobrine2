...
public class Path {
    ...
    private final int maxSteps = Utils.getRandomGen().nextInt(3) + 3;
    protected HerobrineOld PluginCore;
    private boolean xNegative;
    private boolean zNegative;
    private boolean isCompleted = false;
    private int stepNow = 0;

    public Path(float _x, float _z, HerobrineOld plugin) {

        PluginCore = plugin;

        x = _x;
        z = _z;
        xNegative = (x - PluginCore.HerobrineNPC.getBukkitEntity().getLocation().getX()) < 0;
        zNegative = (z - PluginCore.HerobrineNPC.getBukkitEntity().getLocation().getZ()) < 0;
    }
